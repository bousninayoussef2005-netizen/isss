...
- **Phase 2 — bridge:** **Done (core)** — **`serial_bridge.py`** live → Firestore; **next:** **`systemd`** so the bridge survives reboots (**[PHASE2.md](./PHASE2.md)** Step 7a, `smartlib-serial-bridge.service.example`). **Phase 3+:** kiosk borrow/return rules from `rfid` + `fsr` (not scripted in repo yet).
